...
本文将聚焦于质量在 30 MeV 以下的 HNL 粒子，且仅考虑其与 $\nu_\mu$​ 的混合。
...
\end{equation}
其随 HNL 质量的变化在图 \ref{fig:Pion_HNL_polarize} 中绘出。
...
\begin{figure}
  ...
  \vspace{-1.0cm}
  \caption{不同 HNL 质量下 $\pi$ 介子与 $\mu$ 子衰变产生 HNL 的分支比。可以看出在 HNL 质量较小时（< 10 MeV）
...
\begin{figure}
  ...
  $m_N = 20$ MeV。能谱对 $\nu_\mu$ 进行了归一化处理，并令 $|U_{\mu}|^2 = 1$。HNL 的造成了显而易见的低能截断
  和总体的衰变宽度降低，也使得 HNL 倾向于携带更多能量。}
...
\begin{figure}
  ...
  \vspace{-1.0cm}
  \caption{在 $\mu$ 子静止系中，其衰变产生不同质量 HNL 的左旋和右旋比例。与 \ref{fig:Pion_HNL_polarize} 中的
  趋势不同，HNL 粒子的左旋与右旋产率在质量变大后快速上升至接近相等，这是因为三体衰变为角动量守恒创造了更灵活
  的条件。}
...
的分支比。而由于 $\pi$ 介子衰变得到的 $\mu$ 子本身带有极化，这使得分析会极为复杂。
...
\end{equation}
其中 $s$ 为螺旋度，且 $+1,-1$ 分别表示 HNL 粒子为右旋与左旋。代入 \label{eq:g_e_p2} 和 \label{eq:g_e_p3} 后积分可以得到
\begin{equation}
  \int_{E}^{+\infty} \frac{d E_N}{E_N} \, Q_N^{(Lab)}(E_N) [16 y^3 - 27 y^2 + 11 -  s \cdot (32 y^3 - 63 y^2 + 36 y - 5) ]
\end{equation}
其中 $y=E/E_N$，在此式中弱混合角的参数被扺消了。对于不考虑 HNL 螺旋度的情况，取 $s=0$ 即可达到对螺旋度平均的效果。
