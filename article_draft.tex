...
\begin{abstract}
  ...
  本文首先通过 Inversed Seesaw （ISS）机制引入HNL，详细计算了在AGN高能环境中，不同质量的 HNL 通过$\pi$介子和$\mu$子衰变的产生时，
  相比较于产生标准模型中微子的分支比，以及产生 HNL 粒子在母粒子静止系下的螺旋度分布、不同螺旋度的 HNL 粒子在轻子衰变与辐射衰变通道中产物的能谱分布。
...
本文的创新点包括：
计算了在母粒子静止系下，产生的 HNL 螺旋度分布。计算了考虑自旋的 HNL 轻子和辐射衰变产物的角分布，并进一步推导出不同螺旋度的 HNL
...
衰变产生的具体途径与运动学特征。最后，2.3节将计算HNL的寿命及其主要衰变通道，包括轻子衰变与辐射衰变，
...
中，产生螺旋度 $-1$ 的 HNL 粒子与 $+1$ 的 HNL 粒子的宽度由下式给出
\begin{equation}
...
\begin{figure}[htbp]
  ...
  \caption{NGC 1068 经 HNL 输运的 $\gamma$ 射线估算结果（基于辐射衰变主导假设）。}
...
\begin{figure}[htbp]
  ...
  \caption{NGC 1068 经 HNL 输运的 $\gamma$ 射线估算结果（仅考虑 ISS 机制轻子衰变）。}
