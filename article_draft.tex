...
\begin{abstract}
  ...
  本文首先通过 Inversed Seesaw （ISS）机制引入HNL，详细计算了在AGN高能环境中，HNL通过$\pi$介子和$\mu$子衰变的产生途径、
  运动学特征以及随后的轻子衰变与辐射衰变通道。随后，将该理论模型应用于 NGC 1068 的实际观测数据中，估算了 HNL 输运并转化
  ...
  \vspace{1em}
  \noindent \textbf{关键词：} HNL, AGN, NGC 1068, 多信使天文学, ISS
\end{abstract}
...
随着天文观测技术的不断进步，更多的信号被纳入到天文观测的对象中。除了传统的可见光，无线电波等等各种波段的电磁波
...
另一方面，在 IceCube 对于活动星系核 NGC 1068 的观测中，与 \cite{murase2020hidden} 等理
论预测相比对时，可以发现高能
...
来之后再变为可探测的光子信号。一个可能的路径是类轴子粒子（ALP），在文章 \cite{pant2024constraining} 中有所讨论。
...
\begin{figure}[htbp]
  ...
  \caption{基于 NGC 1068 冕区电磁级联模型的 $\gamma$ 射线通量预测，引用自 \cite{murase2020hidden}。图中黑线为对于
  中微子通量的不同预测，红色为对 $\gamma$ 射线通量的理论预测。可以看到电磁级联模型良好地解释了较低能量的 $\gamma$
  射线，但对于更高能的部分则低于现有的实验观测。}
...
性轻子（HNL）也存在可能。在解释中微子质量起源的模型中，如 Type-I Seesaw 和 Inverse Seesaw 模型，都会引入 HNL 粒
子 \cite{king2025righthandedneutrinosseesawmodels}。通过天文观测结果寻找这些粒子的潜在信号，是一个能够发挥天文现
象的尺度优势，与地面实验室结果形成互补的现象学研究。

本文将如此安排：
第二部分将讨论 HNL 的理论基础，包括 Type-I Seesaw 机制和 Inversed Seesaw（ISS）机制，从中导出的 HNL 产生与衰变
过程，以及额外的偶极衰变（直接 $\gamma$ 射线衰变）。第三部分将讨论 AGN 中产生 HNL 粒子并衰变产生可观测 $\gamma$ 射线信号的可能性，
结合 NGC 1068 的观测数据，尝试解释多信使观测结果，并限制 HNL 的参数空间。最后在第四部分得出结论。

\newpage
\section{HNL 的理论基础}
在探讨 HNL 在天体物理环境中的可观测信号前，本章将讨论 HNL 的理论框架。首先，在 2.1 节中我们将通过 Inverse Seesaw （ISS）机制
引入 HNL，解释其中微子微小质量的起源及HNL混合角特性。随后，2.2 节将详细推导在AGN的高能碰撞环境中，HNL 通过 $\pi$ 介子和 $\mu$ 子
衰变产生的具体途径与运动学特征（包括能谱与螺旋度不对称性）。最后，2.3节将计算HNL的寿命及其主要衰变通道（轻子衰变与辐射衰变），
这决定了 HNL 粒子能否逃逸 AGN 的遮蔽区域并产生我们在地球上可观测到的 $\gamma$ 射线。

\subsection{HNL 起源：ISS 机制}
...
\end{equation}
引入 ISS 机制后，由于多引入了一个表示轻子数坏的 $\mu$ 参数，使得混合角与 HNL 粒子的质量解耦。
因此可以在自然地赋予中微子微小质量的同时，产生可观测的 HNL 混合角与质量。

\subsection{HNL 的产生途径}
...
%   \vspace{-1.0cm}
%   \caption{不同 HNL 质量下经由 $\pi$ 介子衰变产生右旋和左旋 HNL 的比。对于反 HNL 粒子，此过程是 CP 对称的。除非 HNL 的质量非常接近相空间边缘，HNL 
%   倾向以“正确的螺旋度”产生，取决于 HNL 的质量，2/3 以
% 上至几乎全部的 HNL 粒子是左旋的。}
...
后的简果在图 \ref{fig:spec_muon_HNL_pol} 中绘出。
对于反 HNL 粒子，此过程是 CP 对称的。可以看出除非 HNL 的质量非常接近相空间边缘，HNL 
...
\begin{figure}[htbp]
  ...
  \vspace{-1.0cm}
  \caption{母粒子静止系下，不同 HNL 质量下经由 $\pi$ 介子和 $\mu$ 子衰变产生右旋和左旋 HNL 的比。}
...
假设在实验室系中 HNL 具有确定的螺旋度，则室验室系中得到的电子谱线可以卷积得到为
\begin{equation}
...
另一方面，考虑近似情况，若认为所有的 HNL 都是左旋的，则 $\gamma$ 谱线可以直接由 HNL 谱得到（取高度相对论极限）
\begin{equation}
...
\end{equation}
上式其实式 \ref{eq:hnl_spec_from_pion} 的简化版本，因为此处的衰变产物的质量在计算过程中被忽略，所以 $\lambda$ 因子被简化了。
...
\subsection{NGC 1068 的多信使观测现状与物理背景}
...
HNL 中介的 $\gamma$ 能谱（图 \ref{fig:gamma_spec_U_2}）。图中红色实线为估算结果，其它是来自文献 \cite{icecube2022evidence}
的理论值和观测结果。参数取值为
...
背景微波的逆康普顿散射，产生 $\gamma$ 射线。同样地，可以忽略产生的 HNL 的极化，直接对两极化平均。对于产生电子如何转化为 $\gamma$ 射线，
可以使用简单的 $\delta$ 假设，即某一能量的 $\gamma$ 光子完全来自于相应能量的电子。能量关系可以表示为
\begin{equation}
...
\begin{figure}[htbp]
  ...
  \caption{NGC 1068 经 HNL 输运的 $\gamma$ 射线估算示意图（基于辐射衰变主导假设）。}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{julia_plots/u_e_gamma.pdf}
  \caption{NGC 1068 经 HNL 输运的 $\gamma$ 射线估算示意图（仅考虑 ISS 机制轻子衰变）。}
...
本文针对质量在 30 MeV 以下的HNL粒子进行了详细的唯象学计算与唯象模型应用，得出以下主要结论：
\begin{enumerate}
  \item 推导了AGN高能背景下，HNL经由$\pi$介子和$\mu$子衰变的产生分支比及能谱特征，发现 HNL 由于质量导致的截断效应使其倾向于携带更高的能量；
  \item 评估了 HNL 的轻子衰变通道。在仅考虑反演跷跷板（ISS）机制的情况下，由于电子衰变受抑制及转换通量的损失，产生的$\gamma$射线信号极其
  微弱，在当前及未来的观测精度下均难以被探测；
  \item 若引入额外的有效算符使得HNL偶极辐射衰变占主导，HNL在逃逸出AGN冕区后可以直接转化为 $\gamma$ 光子。利用这一机制计算所得的 $\gamma$ 射线
  谱能够在一定程度上匹配观测结果，并以此推断给出了HNL混合角 $|U_\mu|^2$ 在特定质量范围内的上限限制。
\end{enumerate}
...
% 本文详细地讨论了 HNL 的与活动星系核相关的唯象学，包括在 $\pi$ 介子与 $\mu$ 子衰变过程中的产生，以及轻子衰变或者辐射
% 衰变产生的可观测信号。如果 HNL 粒子缺乏直接的辐射衰变过程，那么由于轻子通道中电子衰变的抑制和转换为可观测 $\gamma$
% 射线时的通量损失，在现有的观测精度和未来的精度下都无法观测到 NGC 1068 中来自 ISS 机制 HNL 的 $\gamma$ 射线信号。如果 HNL 存
% 在直接的辐射衰变通道，且辐射衰变通道占主导，那么观测到的信号则存在部分来自于 HNL 的可能性。

% 然而本文内使用了大量的近似，如忽略产生的 HNL 的极化，以及对 HNL 衰变都在冕区外，视界内的假设。另外辐射衰变如果采用单纯的
% 偶极跃迁，其参数空间也已经被超新星的观测实验排除。虽然如此，但在新一代的中微子观测站以及 $\gamma$ 射线天文台建成后，
% 仍有希望压缩参数空间，或是观测到来自 HNL 的信号。
