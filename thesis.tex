...
\ecnuSetup{
  info = {
    title = {重中性轻子的唯象学特性与 {NGC 1068} 多信使观测},
    % 中文标题
    %
    titleEN = {The Phenomenology of HNL and Multi-Messenger Observation of NGC 1068},
    % 英文标题
    %
    author = {韦晾},
    % 作者姓名
...
\begin{figure}[htbp]
  ...
  \caption{基于 NGC 1068 冕区电磁级联模型的 $\gamma$ 射线通量预测，引用自 \parencite{murase2020hidden}。图中黑线为对于
  中微子通量的不同预测，红色为对 $\gamma$ 射线通量的理论预测。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{julia_plots/U_UL.pdf}
...
\begin{appendix}
  ...
  在此呈现一些 HNL 相关费曼图的计算过程，相应过程的费曼图都已在正文中呈现。

  首先考虑 $\pi$ 介子衰变成 HNL 粒子的情况。正文中的费曼图的振幅可以写为
  \begin{equation}
    \mathcal{M} = \frac{f_\pi g_w^2}{8 M_W^2} k^\mu [\bar{u}(p_2) \gamma_\mu (1 - \gamma^5) v(p_1)]
  \end{equation}
  套用求解费曼图的一般过程，即重新排列旋量后使用求迹，即可求出相应的衰变宽度。而如果需要考虑末态 HNL 粒子的
  自旋，则需要使用如下不对自旋平均的旋量替换
  \begin{equation}
  ...
  \end{equation}
  于 $\delta$ 函数的选择性，以及为保证 $\delta$ 函数不为 0，需要
  \begin{align}
  ...
  对于 HNL 粒子衰变的过程，取 HNL 静止系，则自旋四矢量保持 $(0,0,0,1)$，而只需在进行洛伦兹 Boost 时注意对应的角度的正反即可。
  ...
  本文研究的 HNL 衰变成为电子的过程是一弱中性过程，因而顶角会带有弱混合角
  \begin{equation}
